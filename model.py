...
from utils import pad_to, unpad


class SeeInDark(nn.Module):
    def __init__(self, input_channel=1, pooling_size=2):
        ...
        self.pool4 = nn.MaxPool2d(kernel_size=pooling_size)
    ...
    def forward(self, x):
        ...
        pool1, pad1 = pad_to(conv1, self.pooling_size)
        pool1 = self.pool1(pool1)

        conv2 = self.lrelu(self.conv2_1(pool1))
        conv2 = self.lrelu(self.conv2_2(conv2))
        pool2, pad2 = pad_to(conv2, self.pooling_size)
        pool2 = self.pool1(pool2)

        conv3 = self.lrelu(self.conv3_1(pool2))
        conv3 = self.lrelu(self.conv3_2(conv3))
        pool3, pad3 = pad_to(conv3, self.pooling_size)
        pool3 = self.pool1(pool3)

        conv4 = self.lrelu(self.conv4_1(pool3))
        conv4 = self.lrelu(self.conv4_2(conv4))
        pool4, pad4 = pad_to(conv4, self.pooling_size)
        pool4 = self.pool1(pool4)
